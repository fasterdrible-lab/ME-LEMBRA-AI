...
> Última atualização: 2026-08-31 (sessão 24c)
...
### Perfil Idoso (agora chamado "Melhor Idade" — sessão 24, era "Vovô / Vovó")
- [x] Interface com botões grandes (altura 70–90 px, fonte 24–32 px)
- [x] **Botão "Assistente Molly"** (sessão 24, antes "Falar Comando"): abre `MollyAssistantPanel` direto por cima da tela atual (`showModalBottomSheet`, sem navegar), já ouvindo sozinho assim que aparece. Substitui de fato o fluxo abaixo em produção — a lógica antiga (`_falarComando` em `elderly_screen.dart`) continua no código, sem uso, fácil de reverter se precisar. A descrição abaixo documenta esse fluxo antigo (ainda válida como referência do que a Molly nova reproduz/estende — ver TASKS.md MOLLY-20 a MOLLY-24 pro que já foi portado: coleta de dia/hora, frases de encerramento, gatilho de memória, proatividade):
...
| **"Modo Proteção" é morto pelo Android depois de ~6h rodando (Android 15+)** | **Alto** | Descoberto na sessão 24 (2026-08-31), analisando um crash real no log do aparelho físico após alguns dias com o app fechado: `FATAL EXCEPTION: android.app.RemoteServiceException$ForegroundServiceDidNotStopInTimeException: A foreground service of type dataSync did not stop within its timeout: SosProtectionService`. Causa raiz: `SosProtectionService.kt` está declarado como `android:foregroundServiceType="dataSync"` no `AndroidManifest.xml` — a partir do Android 15 (API 35), esse tipo tem um limite de **6h de execução a cada 24h**; ao estourar, o sistema chama `Service.onTimeout()` e exige `stopSelf()` em poucos segundos, senão **mata o app inteiro** à força (confirmado via [developer.android.com/.../fgs/timeout](https://developer.android.com/develop/background-work/services/fgs/timeout)). Como o Modo Proteção existe justamente pra rodar indefinidamente em segundo plano (mantém o detector de queda vivo com o app fechado), `dataSync` nunca foi o tipo correto pra esse caso de uso — só passou a quebrar agora que o Android fiscaliza isso (o aparelho de referência R9QL200MJ0N atualizou pro Android 16/API 36). **Efeito prático**: o detector de queda para de funcionar sozinho, sem avisar o usuário, depois de ~6h com o app fechado e o Modo Proteção ativado. **Correção identificada, não aplicada** (usuário pediu só documentar por ora): trocar `foregroundServiceType` de `dataSync` pra `health` (permissão `FOREGROUND_SERVICE_HEALTH`) — sem esse teto de 6h, e semanticamente mais correto pra monitoramento de emergência/queda — mais implementar `onTimeout()` como rede de segurança adicional. Nada alterado em código nesta entrada. |
...
> **Sessão 24 — chave da Groq inválida (causa diferente do 404 de
> modelo):** testando a Molly nova no aparelho, `journalctl -u
> melembra-ai-backend` mostrou `Error code: 401 - Invalid API Key` (não
> 404 `model_not_found` como nas sessões 19/21) — a `GROQ_API_KEY` salva
> no `.env` da VPS estava sendo recusada pela Groq. Usuário gerou uma
> chave nova e colou no chat; atualizada via `sed` no `.env` (sem expor o
> valor no log) + `systemctl restart melembra-ai-backend`, autorizado
> explicitamente pelo usuário (o classificador automático bloqueia
> escrita+restart em VPS por padrão). `healthz` e um teste real no app
> confirmaram a correção. **Dois motivos diferentes já causaram "Molly não
> responde"** — modelo descontinuado (404) e chave inválida (401) — vale
> checar os dois ao investigar no futuro.
>
> **Sessão 24 — `app.py` ganhou o campo opcional `memoria`** (gatilho de
> memória de longo prazo, MOLLY-23 em `TASKS.md`) — implantado do mesmo
> jeito (`scp` + `systemctl restart`), `healthz` OK.
...
- **Prevenção de falsos positivos na emergência suave** (TAREFA 19,
  sessão 24): `OfflineIntentService._pareceEmergenciaSuave` ganhou três
  guardas antes de reconhecer uma das cinco frases suaves da TAREFA 18.
  (1) Palavra inteira via `_indiceDaFraseInteira()` (usa `\p{L}` Unicode,
  não `\b`/`\w`, porque o motor de regex do Dart trata letras acentuadas
  como "alguém" como não-palavra) — corrige um bug real: "eles me
  ajudaram ontem" casava com "me ajuda" via `.contains` simples, já que
  "ajudaram" começa com "ajuda". (2) Negação (`_precedidaDeNegacao`):
  "não preciso de ajuda, já resolvi sozinho" não dispara mais. (3) Relato
  de passado (`_marcadoresPassado`: "ontem", "semana passada", "no
  passado" etc.): "ontem eu disse que preciso de ajuda, mas já passou"
  não dispara mais. As cinco frases literais sem negação/passado por
  perto continuam disparando a confirmação visual normalmente — nenhuma
  mudança de comportamento aí. 4 testes novos em
  `test/offline_intent_service_test.dart`. `elderly_screen.dart` não foi
  tocado; sem mudança no backend.

## MOLLY — continuação da sessão 24: virou o assistente de produção

Depois da TAREFA 19, a mesma sessão seguiu com quatro frentes pedidas
explicitamente pelo usuário, "faça todas na ordem" — três concluídas, uma
pausada por decisão dele. Resumo (ver `TASKS.md`, MOLLY-20 a MOLLY-24,
para os detalhes técnicos completos):

1. **Coleta local de dia/hora ligada** (MOLLY-20) — `MollyReminderParser`
   porta os extratores de `elderly_screen.dart`; a Molly nova agora
   termina de verdade um pedido de lembrete incompleto, sem depender da
   IA lembrar o que já foi dito. De brinde, corrigido um bug real de
   `ValueNotifier` usado após `dispose()` em `MollyVoiceService`.
2. **"Falar Comando" trocado pela Molly** (MOLLY-20/21/22) — o botão do
   perfil idoso (agora chamado **"Melhor Idade"**, não mais "Vovô / Vovó"
   nem "Modo Idoso") abre a Molly nova **direto**, como
   `showModalBottomSheet` sobre a tela atual (sem navegar), já ouvindo
   sozinha assim que aparece — pedido explícito do usuário pra não perder
   a sensação imediata do botão antigo. `elderly_screen.dart` continua no
   repositório, sem uso, pra reverter fácil se precisar.
3. **Máquina de conversa** (MOLLY-22) — frases de encerramento portadas.
   Achado importante ao revisar o código antigo com calma: ele **nunca**
   reabriu o microfone sozinho "pra qualquer assunto" — só durante a
   coleta de dia/hora, de propósito (evitar capturar silêncio como "não
   entendi" depois de uma interação já resolvida). Isso não é uma
   limitação nova da Molly — é o comportamento já validado que ela
   reproduz.
4. **Gatilho de memória de longo prazo** (MOLLY-23) — a IA pode sugerir
   `memoria: {tipo, valor, confianca}` junto de qualquer resposta; o app
   só pergunta em voz alta (nunca grava sozinho) se o interruptor geral
   estiver ligado. Implantado em produção (`app.py` + `systemctl
   restart`).
5. **Proatividade** (MOLLY-24) — `MollyProactiveService` avisa remédio
   atrasado (tipo, não confirmado, >30 min) assim que a Molly é aberta,
   sem o usuário precisar perguntar.

**Pausado por decisão do usuário**: voz em tempo real (LiveKit, Fase 7 do
prompt mestre) — explicitamente marcada no plano original como "branch
experimental, sem tocar main", exige conta/infraestrutura LiveKit
(Cloud pago ou servidor próprio) e trocar toda a arquitetura de voz
(turnos → áudio contínuo bidirecional). Perguntado ao usuário como
prosseguir; ele escolheu parar por hoje. Fica registrado como próximo
passo, mas nenhum código foi escrito pra isso.

**Ainda pendente, sem mudança nesta sessão**: implementação real da wake
word (bloqueada até decisão sobre conta Picovoice + avaliação de bateria);
o "molly_controller.dart" mencionado em sessões anteriores como um
controller futuro pra máquina de conversa mais ampla acabou não sendo
necessário — a lógica ficou toda em `MollyAssistantPanel`, sem precisar
de uma camada nova. Ver seção "Plano de migração" em
`docs/MOLLY_ARCHITECTURE_ANALYSIS.md` para o mapeamento fase a fase
original (algumas fases já saíram na frente da ordem original, por pedido
direto do usuário nesta sessão).
...
|---|---|---|
| 24c | 2026-08-31 | Reconectado o app no aparelho físico (3 dias depois da 24b) pra retomar o acompanhamento de log — achado um crash real no meio do caminho: `ForegroundServiceDidNotStopInTimeException` matando o processo inteiro. Investigado e documentado na tabela "Lacunas e limitações conhecidas" (ver acima): o Android 15+ limita serviços em primeiro plano do tipo `dataSync` a 6h de execução por dia, e `SosProtectionService` (Modo Proteção) está declarado com esse tipo — o detector de queda para de funcionar sozinho depois desse tempo com o app fechado, sem avisar o usuário. Correção identificada (trocar pra `foregroundServiceType="health"`) mas **não aplicada** — usuário pediu só documentar e subir pro GitHub nesta sessão, sem mexer no código do bug agora. |
| 24b | 2026-08-28 | **Testou o APK no aparelho físico → achou e corrigiu um bug real de produção → depois fez a Molly virar o assistente de produção, em 4 frentes pedidas em sequência pelo usuário.** Ao testar MOLLY-19 no aparelho, a Molly "não respondia" — investigado via log ao vivo (`flutter run` + logcat): a chave `GROQ_API_KEY` na VPS estava retornando 401 Invalid API Key (chave revogada/desatualizada, achado diferente do padrão de "modelo descontinuado" das sessões 19/21). Usuário colou uma chave nova no chat; atualizada no `.env` da VPS via SSH (autorizado explicitamente) e `systemctl restart melembra-ai-backend` — confirmado saudável. Depois disso, usuário pediu pra **substituir o "Falar Comando" pela Molly nova** — identificado um gap real antes de trocar (coleta de dia/hora não estava ligada na Molly nova) e resolvido primeiro (MOLLY-20: `MollyReminderParser`, mais um bug real de `ValueNotifier` pós-`dispose()` corrigido em `MollyVoiceService`), só então o botão foi trocado. Usuário pediu a assistente **"acionada direto, sem ir pra outra tela"** — extraído `MollyAssistantPanel` de `molly_screen.dart` (MOLLY-22), reaproveitado como `showModalBottomSheet` que já começa a ouvir sozinho, sem `Navigator.pushNamed`. Usuário pediu renomear "Modo Idoso" pra "Melhor Idade" (MOLLY-21) — achado que o rótulo é também o valor **armazenado** no Firestore/SharedPreferences (não só um texto de tela), então a troca ganhou normalização de compatibilidade pra contas já existentes, sem forçar reseleção de perfil. Perguntado "quais tarefas pendentes" → usuário pediu as 4 frentes restantes do plano da MOLLY **"todas na ordem"**: máquina de conversa (MOLLY-22 — achado que o código antigo nunca reabria o microfone "pra qualquer assunto", só durante coleta de dia/hora; comportamento replicado fielmente, não inventado), gatilho de memória de longo prazo (MOLLY-23, com deploy de backend autorizado), proatividade (MOLLY-24, remédio atrasado). A quarta frente (voz em tempo real via LiveKit) foi pausada por decisão explícita do usuário depois de eu explicar que é uma mudança de infraestrutura/conta, marcada desde a análise original como "branch experimental, sem tocar main". Ver `TASKS.md` (MOLLY-20 a MOLLY-24) para os detalhes técnicos de cada frente. `flutter analyze` limpo em todas; `flutter test` sem regressão em nenhuma etapa (101 passando ao final / 6 falhando pré-existentes, mesmos de sempre). Testado no aparelho físico (R9QL200MJ0N) a cada etapa, reinstalação completa via `flutter run` (não hot-reload) por segurança. |
| 24 | 2026-08-28 | **MOLLY-19 — prevenção de falsos positivos na emergência suave.** `OfflineIntentService._pareceEmergenciaSuave` (TAREFA 18) usava `.contains` puro nas cinco frases suaves, o que tinha um bug real de falso positivo (não só hipotético): "eles me ajudaram ontem" casava com "me ajuda", pois "ajudaram" começa com "ajuda". Corrigido com três guardas: (1) `_indiceDaFraseInteira()` exige palavra inteira — usa `\p{L}` (Unicode) em vez de `\b`/`\w` porque o motor de regex do Dart trata letras acentuadas (ex.: "alguém") como não-palavra por padrão, o que quebraria `\b` bem no meio da frase; (2) `_precedidaDeNegacao()` suprime quando "não"/"nao"/"nunca"/"nem" aparece nas até três palavras antes da frase; (3) lista `_marcadoresPassado` ("ontem", "semana passada", "no passado" etc.) suprime frases que relatam algo do passado já resolvido — o cenário que o `CURRENT_STATE.md` já citava como pendente desde a TAREFA 18. As cinco frases literais continuam disparando normalmente quando não há negação/passado por perto — sem mudança de comportamento no caminho feliz. `elderly_screen.dart` não foi tocado; sem mudança no backend (`app.py`), nada pra implantar na VPS. 4 testes novos em `test/offline_intent_service_test.dart`; `flutter analyze` limpo; `flutter test` sem regressão (74 passando incluindo os 4 novos / 6 falhando pré-existentes, mesmos de sempre em `reminder_service_test.dart`). |
