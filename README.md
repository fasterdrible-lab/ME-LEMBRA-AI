...
## Funcionalidades já implementadas (2026)

- Tela dedicada para Idoso (acessível): interface com botões grandes, texto ampliado, botão SOS, entrada e leitura por voz.
- Entrada por voz (speech-to-text) e leitura automática de lembretes (text-to-speech) para o perfil Idoso.
- Seleção de perfil (Idoso, Adulto, Criança) com navegação dedicada.
- Criação, edição e exclusão de lembretes por categoria (remédio, consulta, aniversário, mercado, reunião, tomar água).
- Listagem de lembretes do dia e futuros.
- Notificações locais agendadas para lembretes.
- Confirmação de lembrete realizado.
- Migração de dados antigos do SharedPreferences para Firestore.
- Interface amigável, com header gradiente, ícones por categoria, badges, FAB para adicionar lembrete, barra inferior de navegação.
- Data formatada em português.
- Botão SOS (dialog visual).
- Tela de monitoramento para Criança (placeholder).
- Serviços de voz e notificações integrados.

Consulte o arquivo DEVLOG.md para histórico técnico e detalhes de problemas resolvidos.
