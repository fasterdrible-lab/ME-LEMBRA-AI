...
class _EditReminderScreenState extends State<EditReminderScreen> {
  ...
  void initState() {
    ...
    _tituloController = TextEditingController(text: r.title);
    _descController = TextEditingController(text: r.description);
    tipo = r.type;
    selectedDate = r.dateTime;
    selectedTime = TimeOfDay(hour: r.dateTime.hour, minute: r.dateTime.minute);
    repeat = r.repeat;
  }
  ...
  Future<void> _salvar() async {
    ...
    try {
      ...
      await ReminderService.update(updated);

      // Cancel old notification and schedule a new one
      final oldNotifId =
          (widget.reminder.title.hashCode ^ widget.reminder.dateTime.millisecondsSinceEpoch)
              .remainder(2147483647)
              .abs();
      await NotificationService.cancel(oldNotifId);

      final newNotifId =
          (titulo.hashCode ^ dateTime.millisecondsSinceEpoch)
              .remainder(2147483647)
              .abs();
      if (repeat == 'unico') {
  ...
  String _getNotificationTitle(String tipo) {
    switch (tipo) {
      case 'Remedio':
        return '💊 Hora do remédio!';
      case 'Consulta':
        return '🩺 Você tem uma consulta!';
      case 'Aniversario':
        return '🎂 Aniversário hoje!';
      case 'Mercado':
        return '🛒 Lista de compras';
      case 'Reuniao':
        return '🤝 Você tem uma reunião!';
      case 'Tomar':
                ...
                onPressed: _salvando
                    ...
                    : () async {
                        ...
                        if (confirmar == true) {
                          setState(() => _salvando = true);
                          try {
                            final notifId = (widget.reminder.title.hashCode ^ widget.reminder.dateTime.millisecondsSinceEpoch)
                                .remainder(2147483647)
                                .abs();
                            await ReminderService.delete(widget.reminder.id, notificationId: notifId);
                            if (mounted) Navigator.pop(context);
                          } catch (e) {
                            if (mounted) {
                              ScaffoldMessenger.of(context).showSnackBar(
