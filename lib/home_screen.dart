...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _carregarPerfil() async {
    ...
    if (perfil == 'Adulto') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) Navigator.pushReplacementNamed(context, '/adult');
      });
      return;
    }
    if (perfil == 'Filhos') {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) Navigator.pushReplacementNamed(context, '/child');
      });
      return;
    }
    if (perfil == 'Família') {
                ...
                if (_perfil == 'Vovô / Vovó')
                  Padding(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text('Ler próximo lembrete'),
                      onPressed: () async {
                        final lembretes = await ReminderService.getAll();
                        Reminder? proximo;
                        for (final l in lembretes) {
                          if (l.dateTime.isAfter(DateTime.now())) {
                            proximo = l;
                            break;
                          }
                        }
                        proximo ??= lembretes.isNotEmpty ? lembretes.first : null;
                        if (proximo != null) {
                          final texto = 'Próximo lembrete: ${proximo.title}, ${proximo.description.isNotEmpty ? proximo.description : ''}, às ${proximo.dateTime.hour.toString().padLeft(2, '0')}:${proximo.dateTime.minute.toString().padLeft(2, '0')}.';
                          // ignore: use_build_context_synchronously
                          try {
                            await VoiceService.speak(texto);
                          } catch (_) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('Falha ao usar voz. Verifique permissões.')),
                            );
                          }
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Nenhum lembrete encontrado.')),
                          );
                        }
                      },
                    ),
