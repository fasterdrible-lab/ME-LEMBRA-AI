...
class FamilyService {
  ...
  static Future<String> getOrCreateInviteCode() async {
    ...
    // Fire-and-forget: sincroniza nome/perfil no convite para que linkWithCode
    // não precise ler users/{ownerUid} (evita permission-denied).
    // Nunca bloqueia nem lança exceção.
    _db.collection('invites').doc(code).set({
      'uid': user.uid,
      'nome': nome,
      'perfil': perfil,
  ...
  static Future<FamilyMember> linkWithCode(String code) async {
    ...
    // Nome e perfil vêm do convite para evitar leitura de users/{ownerUid}
    // (regra Firestore só permite após vínculo existir).
    final ownerNome = (inviteData['nome'] as String?) ?? 'Familiar';
    final ownerPerfil = (inviteData['perfil'] as String?) ?? '';
