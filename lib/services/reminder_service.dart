...
class ReminderService {
  ...
  static Stream<List<Reminder>> stream() {
    ...
    return _col
        ...
        .map((list) {
          WidgetService.update(list); // fire-and-forget: atualiza AppWidget
          return list;
        });
  }
  ...
  // Confirmar lembrete (ex: "remédio tomado") e notificar familia no chat
  static Future<void> confirm(String id) async {
    if (_userId == null) return;

    // Ler dados do lembrete antes de confirmar
    Map<String, dynamic>? reminderData;
    try {
      final doc = await _col.doc(id).get();
      if (doc.exists) reminderData = doc.data();
    } catch (_) {}

    await _col.doc(id).update({'confirmed': true});

    // Notificar familiares vinculados (nao bloqueia em caso de erro)
    if (reminderData != null) {
      try {
        final members = await FamilyService.stream().first;
        if (members.isNotEmpty) {
          final title = reminderData['title'] as String? ?? '';
          final myName = await _db
              .collection('users')
              .doc(_userId)
              .get()
              .then((s) => (s.data()?['nome'] as String?) ?? 'Familiar');
          final msg = '💊 $myName confirmou: \'$title\'';
          for (final member in members) {
            await ChatService.send(member.uid, msg);
          }
        }
      } catch (_) {
        // Falha na notificacao familiar nao cancela a confirmacao
      }
    }
  }
